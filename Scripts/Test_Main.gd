...
func _ready():
	...
	if parent_path:
		...
		LoadInstance("BottomEnter", Vector2(-2*room_unit, -2*room_unit))
		LoadInstance("TopEnter", Vector2(-2*room_unit, 0))
		LoadInstance("RightEnter", Vector2(-6*room_unit, -1*room_unit))
